            ...
            <section className={`${styles.home}`}>
                ...
                <div className={`${styles.homeContainer}`}>
                    ...
                    <div className={`${styles.top__body}`}>
                        <h3 className={`${styles.top__title}`}>{t("Top Up")}</h3>
                        <div className={`${styles.choose}`}>
                            ...
                            <div>
                                ...
                                <label htmlFor="cash">
                                    <img alt='' src={cash} />
                                    <span>{t("Trauxit Cash")} </span>
                                </label>
                            </div>
                        </div>
                        <form onSubmit={form.onSubmit(console.log)}>
                            <div className={`choose__input`}>
                                <TextInput label={t("Amount")} placeholder="EX:$500" {...form.getInputProps('name')} />
                                <TextInput mt="sm" label={t("Name Of Card")} placeholder={t("Name Of Card")} {...form.getInputProps('email')} />
                                <div className={`${styles.card__input} card__input`}>
                                    <img alt='' src={card} className={`${styles.card__img}`} />
                                    <TextInput mt="sm" label={t("Card Number")} placeholder="0000 0000 0000 0000" {...form.getInputProps('number')} />
                                </div>
                            </div>
                            <div className={`${styles.date} date`}>
                                <TextInput label={t("Expiry Date")} placeholder="MM/YY" {...form.getInputProps('name')} />
                                <TextInput label={t("Security Code")} placeholder="Cvv" {...form.getInputProps('email')} />
                            </div>
                            <div className={`${styles.remember}`}>
                                <p>{t("Remember this Card for future Use")}</p>
                                <label class="switch">
                                    <input type="checkbox" />
                                    <div class="slider">
                                        <div class="circle">
                            ...
                            <div className={`${styles.security}`}>
                                ...
                                <p>{t("TRAUXIT is Committed to protecting your payment information and only shares your credit card information with our payment service provider who agreed to safeguard your information")}</p>
                            </div>
                            <h3 className={`${styles.term}`}>{t("Terms & Condition")}</h3>
